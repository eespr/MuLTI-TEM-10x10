...
MODULE MEX_LEROI
  ...
  SUBROUTINE ERRORMESSAGE(MSGTYPE, MSG)
    ...
    ERRTYPE = 'MATLAB:' // MEXNAME // ':' // MSGTYPE
...
SUBROUTINE MEXFUNCTION(NLHS, PLHS, NRHS, PRHS)
  ...
  ALLOCATE(TXON(INT(NSX)))
  TXON = (/0.0, 0.001, 1.0492, 1.05/)
  ALLOCATE(WAVEFORM(INT(NSX)))
  WAVEFORM = (/0.0, 1.0, 1.0, 0.0/)
  ! 0.006000  0.007625
  ! 0.007625  0.009750
  ! 0.009750  0.012500
  ! 0.012500  0.015880
  ...
  ! SXZ values do not seem to be read from Leroi.cfl file.
  ALLOCATE(SXED(INT(MXVRTX), INT(NTX)))
  SXED = RESHAPE((/5, -5, -5, 5/), SHAPE(SXED))
  ALLOCATE(SXND(INT(MXVRTX), INT(NTX)))
  SXND = RESHAPE((/5, 5, -5, -5/), SHAPE(SXND))
