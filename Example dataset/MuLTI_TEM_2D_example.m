...
% The physical model consists of internal layers, each with an associated resistivity (R) sampled log(R), defined by Voronoi nuclei.
...
cmp_s = 0; %offset of starting sounding
cmp_e = 88; %offset of last sounding
bin = 4; %sounding moveed along line 
no_curves = (((cmp_e - cmp_s) / bin) + 1); % number of 1D soundings along 2D line

offsets = linspace(cmp_s+(bin/2),cmp_e+(bin/2),no_curves); %offsets of 2D line
...
%matrix of snow and ice depths at each cmp location along the line, length 
%should be equal to no_curves.
...
if num_layers == 3
    ...
    priors.layer_depths(2) = ice(1,c); % layer 2 depth.
    ...
    if out==1
        ...
        if priors_OK == 0 
           ...
           Res = zeros(length(R),7);
            ...
            %%%%%%%%%% computing the forward model %%%%%%%%%%%
            if running_mode == 1
                forward_model = LEROI_TEM(npt_prop+num_layers, 0, npt_prop + num_layers, Res, linspace(1,(npt_prop+num_layers),(npt_prop+num_layers)), thickness);
                forward_model = round(forward_model,6,'significant');  
            %%%%%%%%%% computing the misfit %%%%%%%%%%%
            misfit = NaN(length(data),1);
            for i = 2:length(timegate) % time samples, this should match the time samples.
...
for c = 12 %1:no_curves % gives all the 1D sounding results
