...
%weighting_percent = logspace(0.75,1.35,length(data)).';
weighting = data.*0.05;
...
if priors_OK == 1
    ...
    for i = 2:length(timegate) % time samples, this should match the time samples- miss the first sample.
...
%hold on
%plot(true_model_R,x,'black','LineWidth',2); %only for synthetic tests
hold on
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% R plot (Normalised, log scale, true R)
R_edge_true = exp(R_edge);
best_model_R_true = exp(best_model_R);
inbetween_true = exp(inbetween);
R_mode_n0_true = exp(R_mode_n0);
AV_true = exp(AV(:,1));
%load('true_modelref.mat')
figure
contourf(R_edge_true, depth_edge,CI_density_limitN, 'LineColor', 'none') %pdf plot
%hold on
%plot(true_model,x,'k','LineWidth',2); %best solution
hold on
...
hold on;
plot(timegate_centre, forward_model_best, 'r');
set(gca, 'XScale', 'log')
set(gca, 'YScale', 'log')
% Create ylabel
...
%plot the ensemble of models accepted
if running_mode == 1
figure;
plot(timegate_centre, data, '*');
hold on;
